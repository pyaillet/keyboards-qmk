# dweeep qmk firmware

## What's this?

This repo is my customization of the design of the dweeep keyboard by Sadek Baroudi: https://github.com/sadekbaroudi/sweep36

Here you will find:

- The QMK configuration with a regular qwerty layout and an [ergo-L](https://ergol.org/) layout (your computer needs to be in extended Qwerty mode)
- The case model I use, both STL and Freecad model sources. The top part was designed with Fusion360 though.

The ergo-l layout:

![ergo-L layout](./imgs/keyboard-layout.png)



What it looks like:

![Tented keyboard](./imgs/dweeep_tented.jpg)
![Flat keyboard](./imgs/dweeep_flat.jpg)
